...
AssetRewrite.prototype.rewriteAssetPath = function (string, assetPath, replacementPath) {
   ...
   * \\s* - Any amount of white space
   * (?: - Start non-capture group matching the end of the path
   * ["\')> ] - Match one of "'( > exactly one time
   * | - or
   * \d+(?:[.]\d+x|[xw]) - Match srcset width/pixel-density descriptors: a positive integer directly followed by 'w' or a positive floating point number directly followed by 'x'
   * ) - End non-capture group
   */

  var re = new RegExp('["\'(=,]\\s*([^"\'()=,]*' + escapeRegExp(assetPath) + '[^"\'()\\>=]*)(\\?[^"\')> ]*)?\\s*\\\\*\\s*(?:["\')> ]|\d+(?:[.]\d+x|[xw]))', 'g');
  var match = null;
  /*
